...
filenames = ["pglib-opf-master/pglib_opf_case14_ieee.m"]
model_flag = 10
...
for filename in filenames:
    ...
    graph = nx.Graph()
    mp.encode_graph(graph, bus_data, gen_data, branch_data,demand,cap,1,1)
    ...
    mod.Params.LogFile = 'anor_test_orig.txt'
    ...
    linear_mod.Params.LogFile = 'anor_test_l1.txt'
    ...
    transport_mod.Params.LogFile = 'anor_test_t1.txt'
    ...
    if model_flag == 8:    
        ...
        for start in neg_flow.nodes():
            for end in neg_flow.nodes():
                if end>start:
                    if nx.has_path(neg_flow,  start, end):
                        sp = list(islice(nx.shortest_simple_paths(neg_flow, start, end, weight = 'branch_CR'),20))
                        sp = nx.utils.pairwise(sp)
                        path_pairs += sp
...
if model_flag == 9:    
    ...
    while n > 0 and len(path_pairs) > 0:
        path = random.choice(path_pairs)
        #path = path_pairs[0]
        path_pairs.remove(path)
        path1 = list(nx.utils.pairwise(path[0]))
    ...
    mod.Params.Presolve = 0
    mod.optimize()
    timing.write('LR+HR+TR Input VIs: ' + str(time.time()-t0) + ' obj:' + str(mod.objVal) + ' ' +
             str(filename) + ' ' + str(SEED) + ' ' + str(pySEED) + ' \n')
    timing.close()
...
if model_flag == 10:
    ...
    while len(cycle_paths) >0:
        ...
        mod.addConstr(mod._bus_angle[end] - mod._bus_angle[start] <= sp_length +
                     (mp_length - sp_length) * (len(sp_e) - gp.quicksum([mod._expansion[edge] for edge in sp_e if edge_status[edge] == 0])))
    mod.write('did_we_add_em_all.lp')
    mod.optimize()